...
--//------------------------------------------------------------------------------------------\\-
    if tonumber(Settings['Made By PekChan'].FPS) then
        setfpscap(Settings['Made By PekChan'].FPS)
    end
--//------------------------------------------------------------------------------------------\\-
    if Settings['Made By PekChan'].PERFORMANCE then
...
--//------------------------------------------------------------------------------------------\\-
if Settings['Made By PekChan'].TITAN.GODV3 then
    pcall(function()
...
--//------------------------------------------------------------------------------------------\\-
  if Settings['Made By PekChan'].TITAN.ENABLED then
	...
	repeat wait() until STAND.Backpack:FindFirstChild("Mask")
		...
		local function rm()
			...
			wide = false
			default = false
			if Settings['Made By PekChan'].TITAN.TALL and wide == false and default == false then
				tall = true
			...
			if Settings['Made By PekChan'].TITAN.WIDE and tall == false and default == false then
				wide = true
				if STAND.Character.Humanoid:FindFirstChild("HeadScale") then
					rm()
					wait(0.6)
			...
			if Settings['Made By PekChan'].TITAN.DEFAULT and tall == false and wide == false then
				default = true
				if STAND.Character.Humanoid:FindFirstChild("HeadScale") then
					rm()
					wait(0.6)
...
--//------------------------------------------------------------------------------------------\\--
    if Settings['Made By PekChan'].FACELESS then
...
--//------------------------------------------------------------------------------------------\\--
    ...
    if Settings['Made By PekChan'].LEGS then
		STAND.Character.RightUpperLeg:Destroy()
		STAND.Character.LeftUpperLeg:Destroy()
    end
--//------------------------------------------------------------------------------------------\\--
if Settings['Made By PekChan'].TRAILS then
    coroutine.wrap(function()
...
--//------------------------------------------------------------------------------------------\\-- 
if Settings['Made By PekChan'].TELEPORTMAIN then
    pcall(function()
...
--//------------------------------------------------------------------------------------------\\--
if Settings['Made By PekChan'].FOLLOWANIM then
    local Glide = Instance.new('Animation', game:GetService("Workspace"))
    Glide.AnimationId = 'rbxassetid://'.. tonumber(Settings['Made By PekChan'].FOLLOWANIM.ID)
    Glide.Name = "Follow"
    ...
    function Moved()
        if OWNER.Character:WaitForChild("Humanoid").MoveDirection.magnitude > 0 then
          if Glide2.IsPlaying == true then
            ...
            Glide2:Play(tonumber(Settings['Made By PekChan'].FOLLOWANIM.SPEED))
          else
            Glide2:Stop(tonumber(Settings['Made By PekChan'].FOLLOWANIM.SPEED))
          end     
       end
...
--//------------------------------------------------------------------------------------------\\--
if Settings['Made By PekChan'].NOCLIP.SynapseX then
    ...
    local x = Settings['Made By PekChan'].NOCLIP.Offset
...
coroutine.wrap(function()
    ...
    local success, err = pcall(function()
        if STAND.Character.BodyEffects.Attacking.Value == true then
          for i,v in pairs(game:GetService('Players'):GetPlayers()) do
              if (v.Character.HumanoidRootPart.Position - STAND.Character.LeftHand.Position).Magnitude <= (Settings['Made By PekChan'].RANGE) then 
                if STAND.Character:FindFirstChildOfClass("Tool") then
                    if STAND.Character:FindFirstChildOfClass("Tool"):FindFirstChild('Handle') then
                        ...
                        firetouchinterest(STAND.Character['LeftHand'], v.Character.UpperTorso, 0)
                            if Settings['Made By PekChan'].LEGS == false then
...
--//------------------------------------------------------------------------------------------\\--
if Settings['Made By PekChan'].ANTIFLING then
    coroutine.wrap(function()
...
--//------------------------------------------------------------------------------------------\\--
coroutine.wrap(function()
    while Settings['Made By PekChan'].AUTOPICKUPCASH == true do
        wait()
